...
class FishingRod extends Tool {
	...
	public function addMoney(Player $player){
        $player->sendMessage("§f†§b§lBạn §bĐã Câu Được §f100 §b xu");
	...
	public function onClickAir(Player $player, Vector3 $directionVector): bool{
		...
		//	$this->addMoney($player);
			...
			$this->addSlot($player, 0.5);	FishingLevel::addFishingExp(mt_rand(1, 3), $player);
								$player->addXp(mt_rand(1, 2),false);
								$player->knockBack($projectile, 0, $player->x - $projectile->x, $player->z - $projectile->z, (0.5/$playerFishingLevel));
								$player->sendMessage(Fishing::getInstance()->lang["fishhasgoneaway"]);
							}
