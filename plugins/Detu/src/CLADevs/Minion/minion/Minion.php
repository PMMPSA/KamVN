...
class Minion extends Human{
    ...
    public function initEntity(): void{
        ...
        $this->setNameTag("§l§eĐỆ TỬ §eCủa §b" . $this->player . "\n§eCẤP: §a" . $this->getLevelM());
        ...
        if($source instanceof EntityDamageByEntityEvent){
            ...
            if($damager->getName() !== $this->player){
                if(!$damager->hasPermission("detu.openall")){
                    $damager->sendMessage(C::RED . "§l§f[§aĐệ Tử§f]§cĐây Không Phải Đệ Tử Của Bạn.");
                    return;
                }}
    ...
    public function entityBaseTick(int $tickDiff = 1): bool{
        ...
        $this->coinapi = Plu::get()->getServer()->getPluginManager()->getPlugin("CoinAPI");
        ...
        if($tile instanceof \pocketmine\tile\Chest){
            ...
            $this->inv = $inv;
        $this->setNameTag("§l§eĐỆ TỬ §eCủa §f" . $this->player . "\n§eCẤP: §a" . $this->getLevelM());
		...
		if($player = Plu::get()->getServer()->getPlayer($this->player)){
            $player->sendPopup("§l§eĐệ tử của bạn yêu cầu cúp §bkim cương");
            return false;
        }
        }
                }
    ...
    public function checkEverythingElse(): bool{
        ...
        if($damage >= 1560){
            if($player = Plu::get()->getServer()->getPlayer($this->player)){
        $player->sendMessage("§l§f[§aĐệ Tử§f]§c Cúp Của Đệ Tử Sắp Hỏng Hãy Fix Để Đệ Tiếp Tục Mine");
            }
        return false;
        }
        $player = $this->getLevel()->getServer()->getPlayer($this->player);
	...
	$xu = 0;
                ...
                $player->sendMessage("§l§6[§eĐệ Tử §6] §ebạn nhận được §a".$xu." §l§aXu§e Từ Đệ Tử");          			                EconomyAPI::getInstance()->addMoney($player, $xu);
                }
						}
    if($inventory->canAddItem($drop)){ return true;}elseif(!$inventory->canAddItem($drop)){ 
        if($player = Plu::get()->getServer()->getPlayer($this->player)){
        $player->sendMessage(C::RED . C::BOLD . "§l§f[§aĐệ Tử§f] §e Đệ Của Bạn Không Bán Được Item Trong Rương");
        return false;
        }
        }
            }elseif(!in_array($block->getId(), Plu::get()->getConfig()->getNested("blocks.cannot"))){
    if($player = Plu::get()->getServer()->getPlayer($this->player)){
        $player->sendMessage(C::RED . C::BOLD . "§l§f[§aĐệ Tử§§f] §cĐệ Của Bạn Không Thể Phá Được Block Này");
    }
        return false;
            }
            return false;
    ...
    public function getChestCoordinates(): string{
        if(!isset($this->getCoord()[1])){
            return C::RED . "§l§f[§aĐệ Tử§f]§c Không Tìm Thấy Kết Nối Rương";
        }
