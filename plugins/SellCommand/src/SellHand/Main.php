					...
					/* Sell All */
						...
						foreach($items as $item){
							if($this->sell->get($item->getId()) !== null && $this->sell->get($item->getId()) > 0){
								...
								$sender->sendMessage(TF::GREEN . TF::BOLD . "(!) " . TF::RESET . TF::GREEN . "Bạn đã nhận được " . TF::RED . "$" . $price . TF::GREEN . " khi bán (" . $item->getCount() . " " . $item->getName() . " với $" . $this->sell->get($item->getId()) . " với mỗi).");
